...
def main_loop():
    ...
    time.sleep(1) # Let sensors get initial reading
    ...
    try:
        while not object_found:
            ...
            # --- Get Initial Right Wall Distance ---
            # Wait until it reads a valid measurement (<100 cm)
            if initial_wall_distance is None:
                if dist_right < 100.0:
                    initial_wall_distance = dist_right
                    print(f"\n---> RECORDED INITIAL WALL DISTANCE: {initial_wall_distance:.1f} cm <---\n")
            else:
                # We have a baseline, so check for sudden drops indicating an object
                # If distance drops by more than 8 cm from the base wall, trigger object sequence!
                if dist_right < (initial_wall_distance - 8.0) and dist_right < 40.0:
                    ...
                    if area > 500:
                        ...
                        if M["m00"] != 0:
                            ...
                            if error < -100:
                                ...
                                cv2.putText(frame, "TRACKING LINE", (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            ...
            # Print specifically the right sensor distance to terminal continuously
            print(f"Right US: {dist_right:.1f} cm")

            # Draw HUD
            if initial_wall_distance:
        ...
        if object_found:
            print("\n*** Initiating Turn Procedure ***")
            # Rotate exactly 90 degrees right

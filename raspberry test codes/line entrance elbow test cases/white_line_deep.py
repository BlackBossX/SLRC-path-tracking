...
# Physical Kinematic Constants for Rotation
WHEEL_DISTANCE_CM = 18.0  # Distance between the left and right back wheels
WHEEL_DIAMETER_CM = 6.5
TICKS_PER_WHEEL_REV = 220.0  # <--- TUNE THIS: Total encoder ticks for exactly 1 full 360-degree spin of the physical wheel.
...
def rotate_in_place(degrees, max_speed=28):
    ...
    # --- Kinematic Math ---
    # 1. How far a wheel needs to travel on the ground to rotate the robot
    # The circle the wheels draw has a diameter equal to the distance between them
    robot_circumference = math.pi * WHEEL_DISTANCE_CM
    target_distance_cm = (abs(degrees) / 360.0) * robot_circumference
    
    # 2. Convert ground distance to wheel revolutions
    wheel_circumference = math.pi * WHEEL_DIAMETER_CM
    wheel_revolutions = target_distance_cm / wheel_circumference
    
    # 3. Convert wheel revolutions to target encoder ticks
    target_ticks = wheel_revolutions * TICKS_PER_WHEEL_REV
        
    print(f"\n--- Kinematic Turn: {degrees} degrees ---")
    print(f"- Target Ground Arc: {target_distance_cm:.2f} cm")
    print(f"- Wheel Revolutions: {wheel_revolutions:.2f}")
    print(f"- Target Encoder Ticks: {target_ticks:.1f}")
